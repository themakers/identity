...
func (sess *Session) StartRegularVerification(ctx context.Context, prov, idn string) (verificationID string, err error) {
	p := sess.manager.prov[prov].internal.type2Ref

	log.Println("PROVIDER", sess.manager.prov[prov], p, p.Info().Name)

	securityCode, identity, err := p.StartRegularVerification(ctx, p.NormalizeIdentity(idn))
	if err != nil {
		return "", err
	}
